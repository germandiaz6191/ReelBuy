...
public partial class MarketplacesIndex
{
    ...
    private async Task<TableData<Marketplace>> LoadListAsync(TableState state, CancellationToken cancellationToken)
    {
    ...
    private async Task DeleteAsync(Marketplace marketplace)
    {
        var parameters = new DialogParameters
            {
                { "Message", string.Format(Localizer["DeleteConfirm"], Localizer["Marketplace"], marketplace.Name) }
            };
