...
public partial class StatusesIndex
{
    ...
    {
        var responseHppt = await Repository.GetAsync<List<Status>>("api/statuses");
        ...
        Statuses = responseHppt.Response!;
    }
    ...
    private async Task DeleteAsync(Status status)
    {
        var result = await SweetAlertService.FireAsync(new SweetAlertOptions
        {
            ...
            Text = string.Format(Localizer["DeleteConfirm"], Localizer["Status"], status.Name),
        ...
        var responseHttp = await Repository.DeleteAsync($"api/statuses/{status.Id}");
